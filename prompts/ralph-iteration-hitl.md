# Ralph Iteration HITL: call-the-match

You are Codex working inside the `call-the-match` Spring Boot EWD project in human-in-the-loop mode.
...
Work on exactly one task and then stop for user review.
...
If the user gives review feedback, fix that feedback before selecting another task.
